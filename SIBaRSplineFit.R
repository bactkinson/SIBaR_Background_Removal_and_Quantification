...
## Function that fits spline to background partitioned points
## Requires original NOx measurements, timestamps, and states returned
## From the partitioning step
sibarSplineFit <- function(poll,timestamps,state,index=rep(1,length(poll))){
  ...
  s1.dataframe <- tibble("Poll"=total.s1.poll,"Dayseconds"=s1.Dayseconds,
                         "Yeardays"=s1.yeardays,"Month"=month(total.s1.time),
                         "Day"=mday(total.s1.time),"Timestamps"=total.s1.time,"Index"=total.s1.index) %>%
  ...
  overall.dataframe <- tibble("Dayseconds"=overall.dayseconds,"Yeardays"=overall.yeardays,
                              "Month"=month(timestamps),"Day"=mday(timestamps),"Index"=index) %>%
    group_by(Month,Day,Index)
